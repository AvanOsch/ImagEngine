...
// BOF - Zappo - ImagEngine - TWO LINES - Set a limit to number of Bestsellers and New products
...
// BOF - Zappo - ImagEngine - TWO LINES - Set a limit to number of Bestsellers and New products
		$data['error_bestseller_limit'] = (isset($this->error['bestseller_limit'])) ? $this->error['bestseller_limit'] : '';
		$data['error_latest_limit'] = (isset($this->error['latest_limit'])) ? $this->error['latest_limit'] : '';
...
// BOF - Zappo - ImagEngine - TWO LINES - Set a limit to number of Bestsellers and New products
		$data['config_bestseller_limit'] = (isset($this->request->post['config_bestseller_limit'])) ? $this->request->post['config_bestseller_limit'] : $this->config->get('config_bestseller_limit');
		$data['config_latest_limit'] = (isset($this->request->post['config_latest_limit'])) ? $this->request->post['config_latest_limit'] : $this->config->get('config_latest_limit');
...
// BOF - Zappo - ImagEngine - Capture and Pass Image Settings
		if (isset($this->request->post['config_image_cart_height'])) {
			$image = $this->request->post;
		} else {
			$image = $this->model_setting_setting->getSetting('config');
		}
		// In case never saved before, skip this stuff...
		$data['image_settings'] = array();
		if (isset($image['config_image_common_cart_width'])) {
			foreach ($image as $key => $value) {
				if (substr($key,0,12) == 'config_image' && !isset($data[$key])) {
					$data['image_settings'][$key] = $value;
				}
			}
		}
// EOF - Zappo - ImagEngine - Capture and Pass Image Settings
...
// BOF - Zappo - ImagEngine - TWO LINES - Set a limit to number of Bestsellers and New products
		if (!$this->request->post['config_bestseller_limit'] || !is_numeric($this->request->post['config_bestseller_limit'])) $this->error['bestseller_limit'] = $this->language->get('error_limit');
		if (!$this->request->post['config_latest_limit'] || !is_numeric($this->request->post['config_latest_limit'])) $this->error['latest_limit'] = $this->language->get('error_limit');
